...
- **Strict framing**: every transport rejects empty lines, embedded LF, and
  payloads above 65,535 bytes on both receive and send paths.
- **Compiled model-interface verification**: deterministic `mirrorcpp-v1`
  generated C++23 ports bind local application adapters to the exact semantic
  digest; strict negotiated replay supports required verification and explicit
  legacy fallback without runtime descriptor interpretation or code loading.
...
## Generated model-interface binding

Generate and check the C++ target from a reviewed Mirrors lock before building
the application:
...
Add `generated/mirrorcpp` to the target's include path and implement the
generated `CounterPort`. At startup, construct an immutable
`CompiledAdapterRegistry` whose key contains the generated digest, an
application-chosen adapter ID, `mirrorcpp-v1`, and
`mirrors.state-computer/v1`. Its factory creates one fresh application port and
generated binding, keeps both alive through `LocalBinding::dispose`, and
returns the generated computer. Pass the corresponding
`CompiledAdapterSelection` to `run_client_negotiated` or
`run_client_with_traces_negotiated`.

The runner sends the generated digest and normalized contract, validates the
first reply before factory construction, rechecks the binding digest and
effective configuration, reuses the ordinary stepping loop, and disposes the
binding exactly once. `ErrorKind::model_interface` carries stable local codes;
structured server negotiation failures remain `ErrorKind::registration`, and
behavioral divergence remains `ErrorKind::step_mismatch`. MirrorCPP is a
compiled-only version-1 client: it does not request or interpret runtime
descriptors.

See `test/integration/real_mirror_test.cpp` for the complete generated Counter
factory and stdio/mTLS examples, and `test/unit/model_interface_test.cpp` for
...
The same real-mirror executable also runs the generated Counter through exact
model-interface verification over stdio and allowlisted mTLS. A wrong digest
and a non-allowlisted mTLS client must make zero adapter/SUT calls; a wrong
observer must still reach ordinary terminal `step_mismatch`.
...
`query_job_result` returns either `JobStatus` or `JobResult`, preserving terminal
outcomes. Existing `query_job` remains a status-only compatibility projection;
terminal success maps to `done`, infrastructure failure to `failed`. Async
exchanges verify requested IDs and submission kinds and close a poisoned
connection. The caller must serialize exchanges on a borrowed transport.
Compiled model-interface verification remains supported; a generic Gate facade
and dynamic descriptor interpreter are not added by this change.
