...
```sh
MIRROR_BIN=/path/to/ModelMirrors ctest --test-dir build
```

With `MIRROR_BIN` set, `real_mirror_hourclock` runs (scenarios:
`run_client_with_traces`, `register`, `gen_traces`, `validate` incl.
`bound=101`, `run_client_explore`, `ExploreSession` scripted session, and a
fault-close check); without it the test skips with exit code 77.
...
- **mTLS**: TLS 1.3 with client certificate + optional SHA-256 fingerprint
  ...
  is always on; there is no “insecure” toggle.
